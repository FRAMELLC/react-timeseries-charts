...
class YAxis extends _react.default.Component {
  ...
  yformat(fmt) {
    if (_underscore.default.isString(fmt)) {
      return (0, _d3Format.format)(fmt);
    } else if (_underscore.default.isFunction(fmt)) {
      return fmt;
    } else {
  ...
  postSelect(style, hideAxisLine, height) {
    ...
    let element = (0, _d3Selection.select)(_reactDom.default.findDOMNode(this)).select("g").selectAll(".tick").select("text");
    Object.entries(valueStyle).forEach(_ref => {
      let [prop, val] = _ref;
      return element.style(prop, val);
    });
    element = (0, _d3Selection.select)(_reactDom.default.findDOMNode(this)).select("g").selectAll(".tick").select("line");
    Object.entries(tickStyle).forEach(_ref2 => {
      let [prop, val] = _ref2;
      return element.style(prop, val);
    });
    (0, _d3Selection.select)(_reactDom.default.findDOMNode(this)).select("g").selectAll(".domain").remove();
    if (!hideAxisLine) {
      element = (0, _d3Selection.select)(_reactDom.default.findDOMNode(this)).select("g").append("line").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", height);
      Object.entries(axisStyle).forEach(_ref3 => {
        let [prop, val] = _ref3;
        return element.style(prop, val);
      });
    }
  ...
  generator(type, absolute, yformat, axis, scale, height, tickCount, min, max) {
    ...
    if (type === "linear" || type === "power") {
      if (tickCount > 0) {
        ...
        axisGenerator = axis(scale).tickValues((0, _d3Array.range)(min, max + max / 10000, stepSize)).tickFormat(d => {
          if (absolute) {
  ...
  renderAxis(align, scale, width, height, showGrid, chartExtent, hideAxisLine, absolute, type, fmt, label, tickCount, min, max) {
    ...
    const axis = align === "left" ? _d3Axis.axisLeft : _d3Axis.axisRight;
    ...
    const labelOffset = align === "left" ? this.props.labelOffset - 50 : 40 + this.props.labelOffset;

    // Axis generator
    const axisGenerator = this.generator(type, absolute, yformat, axis, scale, height, tickCount, min, max);

    // Remove the old axis from under this DOM node
    (0, _d3Selection.select)(_reactDom.default.findDOMNode(this)).selectAll("*").remove();

    // Add the new axis
    this.axis = (0, _d3Selection.select)(_reactDom.default.findDOMNode(this)).append("g").attr("transform", `translate(${x},0)`).attr("class", "yaxis").call(axisGenerator.tickSize(tickSize)).append("text").text(label || this.props.label).attr("transform", "rotate(-90)").attr("class", "yaxislabel").attr("y", labelOffset).attr("dy", ".71em").attr("text-anchor", "end");
    Object.entries(valueStyle).forEach(_ref4 => {
      let [prop, val] = _ref4;
      return this.axis.style(prop, val);
    });
    Object.entries(labelStyle).forEach(_ref5 => {
      let [prop, val] = _ref5;
      return this.axis.style(prop, val);
    });
  ...
  updateAxis(align, scale, width, height, showGrid, chartExtent, hideAxisLine, absolute, type, fmt, label, tickCount, min, max) {
    ...
    (0, _d3Selection.select)(_reactDom.default.findDOMNode(this)).select(".yaxis").transition().duration(this.props.transition).ease(_d3Ease.easeSinOut).call(axisGenerator.tickSize(tickSize));
    this.updateLabel(label);
    this.postSelect(style, hideAxisLine, height);
  }
  updateLabel(label) {
    (0, _d3Selection.select)(_reactDom.default.findDOMNode(this)).select(".yaxislabel").text(label);
  }
  render() {
...
YAxis.propTypes = {
   ...
   */
  type: _propTypes.default.oneOf(["linear", "power", "log"]),
  /**
  ...
  style: _propTypes.default.shape({
    ...
    // esline-disable-line
    ticks: _propTypes.default.object // esline-disable-line
  }),
  /**
